...
<section class="tx-card p-5">
  <div class="flex items-start justify-between gap-3">
    <div>
      <h3 class="font-semibold">{$t("Health check")}</h3>
      <p class="mt-1 text-sm text-[var(--color-text-muted)]">
        {$t("MCP and Actions local/public endpoints and OAuth metadata")}
      </p>
    </div>
    <button
      type="button"
      ...
      onclick={runCheck}
    >
      {busy ? $t("Checking…") : $t("Run health check")}
    </button>
  </div>
    ...
    <ul class="mt-4 grid gap-2">
      {#each items as item (item.label)}
        ...
        >
          ...
          >
            {item.ok ? $t("Passed") : $t("Failed")}
          </span>
        </li>
      {/each}
    </ul>
  {:else if !busy && !error}
    <p class="mt-4 text-sm text-[var(--color-text-muted)]">{$t("No health check has been run.")}</p>
  {/if}
</section>
