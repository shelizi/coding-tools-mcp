...
- 必須用 `pnpm exec tauri build --no-bundle`，不要只用 `cargo build --release`。
- `cargo build --release` 不會把 Vite 生產前端嵌入 exe，執行時會去連 `localhost:1420` 而失敗。
- `tauri build --no-bundle` 會先跑 `pnpm run build`，再把前端靜態檔案包進 Rust 執行檔。

## 前置條件

- Node.js 與 pnpm 可用（若用 scoop 安裝，路徑通常不在預設 PATH）
...
用 `pnpm run desktop:build` 或 `pnpm exec tauri build`（不加 `--no-bundle`），會額外產出 MSI 與 NSIS 安裝包。
