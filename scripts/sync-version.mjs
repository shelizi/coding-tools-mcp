...
export function syncNodeAgentPackage(text, version) {
  const document = JSON.parse(text);
  const current = document.codingTools && typeof document.codingTools === 'object'
    ? document.codingTools
    : {};
  if (current.clientVersion === version) return text;
  document.codingTools = { ...current, clientVersion: version };
  return stringifyJson(document, text);
}

export function syncNodeClientVersionSource(text, version) {
  const newline = text.includes('\r\n') ? '\r\n' : '\n';
  return [
    '// Generated by scripts/sync-version.mjs. Do not edit manually.',
    `export const CLIENT_COMPAT_VERSION = '${version}';`,
    ''
  ].join(newline);
}
...
async function main() {
  ...
  const targets = [
    ...
    {
      path: join(workspace, 'packages', 'node-agent', 'package.json'),
      sync: syncNodeAgentPackage
    },
    {
      path: join(workspace, 'packages', 'node-agent', 'src', 'clientVersion.generated.ts'),
      sync: syncNodeClientVersionSource
    }
  ];
