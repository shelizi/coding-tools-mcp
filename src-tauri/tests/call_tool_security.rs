...
#[test]
fn read_file_rejects_explicit_external_path() {
    let fx = malicious_fixture();
    let out = invoke(
        &ctx_for(&fx.root),
        "read_file",
    ...
    assert_security_or_policy_err(&out);
    assert!(!format!("{out}").contains("TOP_SECRET"));
}

#[test]
fn external_read_tools_reject_directory_listing_and_search() {
    let fx = malicious_fixture();
    let ctx = ctx_for(&fx.root);
    ...
    assert!(!format!("{files_result}").contains("outside-secret.txt"));
    ...
    assert!(!format!("{matches_result}").contains("TOP_SECRET"));
}

#[test]
fn view_image_rejects_explicit_external_path() {
    let fx = malicious_fixture();
    let image_path = fx
        .outside_secret
        .parent()
