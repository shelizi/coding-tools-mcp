...
use crate::workspace::{parse_bind_address, WorkspaceProfile};
...
pub fn validate_service_start(
    ...
    validate_service_auth_exposure(target, service)?;
...
pub(crate) fn validate_service_auth_exposure(
    profile: &WorkspaceProfile,
    service: WorkspaceService,
) -> AppResult<()> {
    let (no_auth, bind_address, tunnel_type) = match service {
        WorkspaceService::Mcp => (
            profile.auth.auth_type == "noauth",
    ...
    let public_bind = !parse_bind_address(bind_address)
        .map_err(AppError::Message)?
        .is_loopback();
    let tunnel_enabled = !matches!(tunnel_type.trim(), "" | "none");
    if public_bind || tunnel_enabled {
        return Err(AppError::Message(format!(
            "{} 使用無驗證模式時只能綁定 loopback，且必須停用公網通道。請改用 OAuth／Bearer API Key，或將綁定位址設為 127.0.0.1 並選擇無通道。",
            service.label()
        )));
    }
    Ok(())
...
mod tests {
    ...
    #[test]
    fn local_noauth_without_tunnel_is_allowed() {
        let mut target = profile("local-noauth", 28_766, 8_788);
        target.auth.auth_type = "noauth".into();
        target.runtime.bind_address = "127.0.0.1".into();
        target.tunnel.tunnel_type = "none".into();
    ...
    #[test]
    fn public_bind_with_noauth_is_rejected() {
        let mut target = profile("public-noauth", 28_766, 8_788);
        target.auth.auth_type = "noauth".into();
        target.runtime.bind_address = "0.0.0.0".into();
        target.tunnel.tunnel_type = "none".into();
        ...
        assert!(error.to_string().contains("無驗證"));
        assert!(error.to_string().contains("loopback"));
    }

    #[test]
    fn tunnel_with_noauth_is_rejected_even_on_loopback() {
        let mut target = profile("tunnel-noauth", 28_766, 8_788);
        target.actions.auth_type = "none".into();
        target.actions.bind_address = "127.0.0.1".into();
        target.actions.tunnel_type = "builtin".into();
        ...
        assert!(error.to_string().contains("公網通道"));
    }
