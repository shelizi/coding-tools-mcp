...
///
/// Downloads use the official GitHub URL first. An explicitly configured
/// mirror is only a fallback, and callers must still verify release integrity.
/// An optional outbound proxy can be layered on top.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
mod tests {
    ...
    #[test]
    fn defaults_missing_server_port_to_https() {
        let profile: FrpProfile = serde_json::from_value(serde_json::json!({
            "id": "p1",
            "name": "WSS FRP",
            "server": "coding-tools.example.com"
        }))
        .expect("FRP profile should use the default port");
