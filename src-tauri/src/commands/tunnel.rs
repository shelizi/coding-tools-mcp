...
pub async fn restart_tunnel(
    ...
    let result = {
        ...
        let was_running = guard.status(&profile, kind, &settings).state != "stopped";
...
pub async fn test_tunnel(
    ...
    let was_tunnel_running = {
        ...
        guard.status(&profile, kind, &settings).state != "stopped"
    };
    ...
    if keep_tunnel || tunnel_type_for(&profile, kind) == "builtin" {
