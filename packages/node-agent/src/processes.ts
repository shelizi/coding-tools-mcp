...
import { normalizeCommandGraphCommands, resolveCommandSpec, validateCommandGraphStructure } from './policy.js';
...
export const COMMAND_GRAPH_RETENTION_MS = FINALIZED_SESSION_RETENTION_MS;
...
function commandTimeoutMaxMs(ctx: ToolContext): number {
...
function packageScriptLooksLongRunning(commandDisplay: string): boolean {
  ...
  if (!packageManager || !value.includes(' run ')) return false;
  return value.split(/\s+/).some(token => /(?:build|portable|package|release|verify|test|check|sync|parity)/.test(token));
}

export function resolvedCommandTimeoutMs(args: JsonObject, commandDisplay: string, timeoutMaxMs = DEFAULT_COMMAND_TIMEOUT_MAX_MS): number {
...
function fingerprint(cwd: string, spec: CommandSpec, args: JsonObject, timeoutMs: number): string {
...
export async function startProcess(
  ...
  const commandFingerprint = fingerprint(cwd, spec, args, timeoutMs);
...
export async function disposeProcessSessions(ctx: ToolContext): Promise<void> {
  ...
  for (const graph of graphs) {
    if (!graph.completedAt && !graph.abortController.signal.aborted) {
      graph.abortController.abort(new Error('server restart'));
    }
  }
  ...
  await Promise.allSettled(graphs.map(graph => graph.completion));
}
...
export function pruneRetainedCommandGraphs(graphs: Map<string, RetainedCommandGraph>, reserve = 0): number {
...
async function waitForGraphSession(graph: RetainedCommandGraph, session: ProcessSession): Promise<void> {
  while (!session.finalizedAt) {
    ...
    try {
      await Promise.race([
        waitForSession(session, session.sequence, 30_000, 'finalized').then(() => undefined),
        aborted
      ]);
    } finally {
      removeAbortListener();
...
async function scheduleCommandGraph(ctx: ToolContext, key: string, graph: RetainedCommandGraph): Promise<void> {
  const launch = (id: string, command: JsonObject & { id: string }) => {
    graph.pending.delete(id);
    graph.startedIds.add(id);
    const task = (async () => {
      try {
        const started = await startProcess(ctx, key, command, graph.abortController.signal);
        const session = started.session;
        ...
        if (graph.abortController.signal.aborted && graph.ownedSessionIds.has(session.id) && !session.finalizedAt) {
          await killProcessTree(session, 'KILL', 'graph_cancelled');
        }
        await waitForGraphSession(graph, session);
        if (graph.abortController.signal.aborted && !graph.ownedSessionIds.has(session.id) && !session.finalizedAt) {
        ...
        const structured = error instanceof ProcessToolError ? error : startupToolError(error);
        graph.results.set(id, {
          id,
          ok: false,
          command_ok: false,
...
async function cancelCommandGraph(runtime: FolderRuntime, graph: RetainedCommandGraph, reason: string): Promise<{ accepted: boolean }> {
  ...
  await Promise.all(sessions.map(session => killProcessTree(session, 'KILL', 'graph_cancelled')));
  return { accepted: true };
}

function commandGraphResultMode(args: JsonObject, graphAction: string): 'full' | 'summary' | 'none' {
  const requested = String(args.result_mode ?? '').trim().toLowerCase();
  if (requested) {
    if (!['full', 'summary', 'none'].includes(requested)) {
      throw new ProcessToolError('INVALID_ARGUMENT', 'exec_many result_mode must be full, summary, or none.', 'invalid_argument', false);
    }
...
export async function runCommandGraph(
  ...
  if (signal?.aborted) {
    throw new ProcessToolError('COMMAND_GRAPH_START_CANCELLED', 'Command graph request was cancelled before it could be retained.', 'runtime', true);
  }
  const runtime = currentFolderRuntime(ctx, key);
  ...
  if (!['run', 'status', 'cancel', 'forget'].includes(graphAction)) {
    throw new ProcessToolError('INVALID_ARGUMENT', 'exec_many action must be run, status, cancel, or forget.', 'invalid_argument', false);
  }
  const resultMode = commandGraphResultMode(args, graphAction);
  ...
  if (args.commands !== undefined) {
    try {
      ...
      throw new ProcessToolError('INVALID_ARGUMENT', error instanceof Error ? error.message : String(error), 'invalid_argument', false);
    }
  }
  ...
  if (requestedOperationId) {
    ...
    if (existing) {
      if (graphAction !== 'run' && commands.length) {
        throw new ProcessToolError('INVALID_ARGUMENT', `exec_many action=${graphAction} does not accept commands.`, 'invalid_argument', false);
      }
      if (graphAction === 'status') {
        return commandGraphSnapshot(runtime, existing, true, 0, 0, graphAction, resultMode);
      }
      if (graphAction === 'cancel') {
        const cancelled = await cancelCommandGraph(runtime, existing, String(args.reason ?? '').trim());
      ...
      if (graphAction === 'forget') {
        if (existing.completedAt === undefined) {
          throw new ProcessToolError('COMMAND_GRAPH_STILL_RUNNING', 'Running exec_many graphs must be cancelled or completed before they can be forgotten.', 'conflict', true, {
      ...
      if (commands.length) {
        ...
        const requestedFingerprint = commandGraphFingerprint(commands, configuration.mode, configuration.maxParallel, configuration.stopOnError);
        if (requestedFingerprint !== existing.fingerprint) {
          throw new ProcessToolError('OPERATION_ID_CONFLICT', 'operation_id is already bound to a different exec_many graph.', 'conflict', false, {
            operation_id: requestedOperationId,
    ...
    if (graphAction !== 'run' || !commands.length) {
      throw new ProcessToolError('COMMAND_GRAPH_OPERATION_NOT_FOUND', 'Retained exec_many graph operation was not found or expired.', 'not_found', false, {
        operation_id: requestedOperationId,
        retention_seconds: COMMAND_GRAPH_RETENTION_MS / 1000,
  ...
  if (graphAction !== 'run') {
    throw new ProcessToolError('INVALID_ARGUMENT', `exec_many action=${graphAction} requires a retained operation_id.`, 'invalid_argument', false);
  }
  if (!commands.length) {
    throw new ProcessToolError('INVALID_ARGUMENT', 'commands or a retained operation_id is required.', 'invalid_argument', false);
  }
  const capacityEvictedGraphCount = pruneRetainedCommandGraphs(graphs, 1);
  if (graphs.size >= MAX_RETAINED_COMMAND_GRAPHS) {
    throw new ProcessToolError('COMMAND_GRAPH_LIMIT_REACHED', 'Too many retained exec_many graph operations are active or awaiting expiry.', 'resource', true, {
      retained_graph_count: graphs.size,
  ...
  const configuration = commandGraphConfiguration(ctx, commands, args);
  const fingerprintValue = commandGraphFingerprint(commands, configuration.mode, configuration.maxParallel, configuration.stopOnError);
  ...
  graph.completion = scheduleCommandGraph(ctx, key, graph);
