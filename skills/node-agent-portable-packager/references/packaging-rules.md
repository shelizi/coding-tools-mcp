...
1. Version ownership
2. Required edition layout
3. Build invariants
4. Launcher contract
5. Verification contract
6. Release decision examples
...
- Build and verify from committed source or clearly report local changes.
...
- Record edition, runtime-bundled status, Git commit, build timestamp, build/runtime versions, architecture, minimum Node major, archive name, and all version-source paths in each manifest.
...
- Set `CTMCP_RESTART_SUPERVISED=1` only when it is not already set.
- Forward remaining command-line arguments to `app\dist\cli.js`.
...
1. Confirm the normal Node Agent verification suite passed once, or identify why it was skipped.
2. Inspect both expanded artifacts and reject unexpected dev dependencies, `.git`, source trees, credentials, or user data.
3. Confirm runtime files are present only in `bundled-node`.
4. Extract both archives to fresh directories whose paths contain spaces.
5. Start each with `--no-browser` on a distinct unused port and isolated data directory.
6. Poll `/health`, require HTTP success, and confirm the reported Agent version.
7. Fetch `/ui` and its JavaScript/CSS assets successfully for both editions.
8. Stop both processes and verify no child or test port remains.
9. Validate every line in each `SHA256SUMS.txt`.
10. Record both final ZIP sizes and SHA-256 values.
